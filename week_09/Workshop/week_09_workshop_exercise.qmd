...
total_hiv_raw = pd.read_csv("hiv_prevalence.csv")
new_hiv_raw = pd.read_csv("hiv_incidence.csv")
```
...
population = pd.read_csv("population_estimates_world_bank.csv")
```
...
```{python}
# Your code here
pop_data = population.query('indicator == "SP.POP.TOTL"')
pop_data = pop_data.drop(columns="indicator")
population_long = pop_data.melt(id_vars="country", var_name="year",value_name="population")
...
def incidence_rate(new_cases,population):
    incidence = new_cases / population * 100000
    return incidence

incidence_rate_vec = np.vectorize(incidence_rate)

def prevalence_rate(total_cases, population):
    prevalence = total_cases / population * 100000
    return prevalence

prevalence_rate_vec = np.vectorize(prevalence_rate)

hiv_data_combined_pop['incidence_rate'] = incidence_rate_vec(hiv_data_combined_pop['new_cases'], hiv_data_combined_pop['population'])

hiv_data_combined_pop['prevalence_rate'] = prevalence_rate_vec(hiv_data_combined_pop['total_cases'], hiv_data_combined_pop['population'])
```
...
```{python}
remove_na = hiv_data_combined_pop.query('~incidence_rate.isna()')
remove_na['country'].unique()
```
...
```{python}
# Your code here
choice = ['Djibouti','Argentina','Rwanda']

hiv_data_mini = hiv_data_combined_pop.query('country.isin(@choice)')
...
px.line(hiv_data_mini, x = "year", y="incidence_rate", color="country", title="Incidence rate of HIV over the time", labels = {"incidence_rate":"Incidence Rate", "year":"Time (year)", "country":"Countries"})
```
...
```{python}
# Your code here
px.line(hiv_data_mini, x = "year", y="prevalence_rate", color="country", title="Prevalence rate of HIV over the time", labels = {"prevalence_rate":"Prevalence Rate", "year":"Time (year)", "country":"Countries"})
```
